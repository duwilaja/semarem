...
function add() { 
    var link = '../backend/Api_task_kategori/in'; 
 ...
 function modal_edit(id)
{
    ...
    $.ajax({
        type: "GET",
        url: "../backend/Api_task_kategori/get?id="+id,
        dataType: "json",
        success: function (v) {
            $('#kategori').val(v[0].task_kategori);
        }
    });
...
function up() { 
    $('#form_edit').submit(function (e) { 
        ...
        $.ajax({
            type: "post",
            url: "../backend/Api_task_kategori/up",
            data : $(this).serialize(),
            dataType: "json",
            success: function (r) {
                if(r.status == 1){
 ...
 function del(id)
 {
    swal({
        ...
        if (isConfirm) {         
            var link = '../backend/Api_task_kategori/del'; 
            $.ajax({
                type: "POST",
                url: link,
                data : {'id' : id},
...
function dt() {
    $('#tabel').DataTable({
        ...
        "ajax": {
            "url": '../backend/Api_task_kategori/dt',
            "type": "POST",
        },
