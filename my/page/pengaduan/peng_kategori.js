...
function add() { 
    var link = '../backend/Api_kategori/in'; 
 ...
 function modal_edit(id)
{
    ...
    $.ajax({
        type: "GET",
        url: "../backend/Api_kategori/get?id="+id,
        dataType: "json",
        success: function (v) {
            $('#kategori').val(v[0].peng_kategori);
        }
    });
...
function up() { 
 ...
 function del(id)
 {
    swal({
        ...
        if (isConfirm) {         
            var link = '../backend/Api_kategori/del'; 
            $.ajax({
                type: "POST",
                url: link,
                data : {'id' : id},
